...
class LRBranch(nn.Module):
    ...
    def __init__(self, backbone,enc_channels):
...
#         print("backbone.enc_channels:",self.backbone.enc_channels)
#         print(len(enc_features))
#         print(f"img shape:{img.shape}")
#         for i,x in enumerate(enc_features):
#             print(f"layer:{i} shape:{x.shape}")
        enc2x, enc4x, enc32x = enc_features[0+1], enc_features[1+1], enc_features[4+1]
...
class HRBranch(nn.Module):
    ...
    def __init__(self, hr_channels, enc_channels):
        ...
        self.conv_hr4x = nn.Sequential(
            Conv2dIBNormRelu(3 * hr_channels + 3 + 16, 2 * hr_channels, 3, stride=1, padding=1),
    ...
    def forward(self, img, enc2x, enc4x, lr8x, inference):
        ...
        concated = torch.cat((hr4x, lr4x, img4x), dim=1)
        hr4x=self.conv_hr4x(concated)
